  ...
  const fetchBanks = async () => {
    try {
      ...
      if (res.data?.success && res.data.data) {
        ...
        setBanks(bankList);
        const cashBank = bankList.find(b => (b.name || '').toLowerCase().includes('cash') || (b.type || '').toLowerCase().includes('cash'));
        const defaultBankId = cashBank ? cashBank.id : (bankList[0]?.id || '');
        setPaymentRows(prev => prev.map(r => r.bankId ? r : { ...r, bankId: defaultBankId }));
      } else {
        setBanks([]);
      }
    } catch (err) {
