...
export function Purchase() {
  ...
  const totalBalance = filteredInvoices.reduce((acc, curr) => acc + (curr.balance !== undefined ? curr.balance : Math.max(0, (curr.totalAmount || 0) - (curr.paidAmount || 0))), 0);
        ...
        <div className="flex-1 bg-[#f0f2f5] overflow-auto custom-scrollbar p-3">
          <div className="max-w-[1200px] mx-auto space-y-3">
            ...
            {filteredInvoices.map((invoice, index) => {
              const paidAmount = invoice.paidAmount !== undefined ? invoice.paidAmount : (invoice.status === 'PAID' ? invoice.totalAmount : 0);
              const balanceAmount = invoice.balance !== undefined ? invoice.balance : Math.max(0, invoice.totalAmount - paidAmount);
              const currentBalance = invoice.customer?.balance || 0;
