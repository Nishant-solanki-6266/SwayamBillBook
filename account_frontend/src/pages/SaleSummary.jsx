...
export function SaleSummary() {
  ...
  const totalPaid = invoices.reduce((acc, curr) => acc + (curr.paidAmount !== undefined ? curr.paidAmount : (curr.status === 'PAID' ? curr.totalAmount : 0)), 0);
  const totalDue = invoices.reduce((acc, curr) => acc + (curr.balance !== undefined ? curr.balance : Math.max(0, (curr.totalAmount || 0) - (curr.paidAmount || 0))), 0);
  ...
  const handleExport = () => {
    const csvContent = [
      ...
      ...invoices.map((inv, idx) => [
        ...
        '0.00',
        (inv.paidAmount !== undefined ? inv.paidAmount : (inv.status === 'PAID' ? inv.totalAmount : 0)).toFixed(2),
        (inv.balance !== undefined ? inv.balance : Math.max(0, (inv.totalAmount || 0) - (inv.paidAmount || 0))).toFixed(2)
      ]),
              ...
              <tbody>
                {invoices.map((inv, idx) => (
                  <tr key={inv.id}>
                    ...
                    <td className="border border-gray-800 py-1 px-2 text-right text-[13px]">₹{(inv.paidAmount !== undefined ? inv.paidAmount : (inv.status === 'PAID' ? inv.totalAmount : 0)).toFixed(2)}</td>
                    <td className="border border-gray-800 py-1 px-2 text-right text-[13px]">₹{(inv.balance !== undefined ? inv.balance : Math.max(0, (inv.totalAmount || 0) - (inv.paidAmount || 0))).toFixed(2)}</td>
                  </tr>
                ))}
