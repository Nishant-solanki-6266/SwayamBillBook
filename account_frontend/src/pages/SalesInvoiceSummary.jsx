    ...
    const fetchData = async () => {
      try {
        ...
        const mappedData = (res.data || []).map(item => ({
          ...
          paidAmt: item.paidAmount !== undefined ? item.paidAmount : (item.status === 'PAID' ? item.totalAmount : 0),
          balance: item.balance !== undefined ? item.balance : (item.status === 'PAID' ? 0 : item.totalAmount),
